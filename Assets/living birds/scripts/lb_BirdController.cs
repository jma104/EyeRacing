...
public class lb_BirdController : MonoBehaviour {
	...
	public float birdFlightSpeed = 70.0f;
	...
	void Start () {
		...
		for(int i=0;i<myBirds.Length;i++){
			...
			myBirds[i].SendMessage("SetFlightSpeed", birdFlightSpeed);
			myBirds[i].SendMessage("SetBirdScale", birdScale);
			myBirds[i].SendMessage("SetCollideWithObjects", collideWithObjects);
