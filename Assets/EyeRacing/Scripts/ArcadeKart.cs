...
namespace KartControl
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public float FinalAcceleration { get; private set; } = 0.0f;
        // can the kart move?
        public bool m_CanMove = true;

        public bool roadIsGazeAware = false;
        ...
        void MoveVehicle(float accelerate, bool brake, float turnInput)
        {
            ...
            bool isBraking = (localVelDirectionIsFwd && brake) || (!localVelDirectionIsFwd && accelerate > 0f);

            // if we are braking (moving reverse to where we are going)
            // use the braking accleration instead
            float finalAccelPower = isBraking ? m_FinalStats.Braking : accelPower;

            FinalAcceleration = finalAccelPower * accelRamp;
            ...
            bool gazeAwareAndOnRoad = roadIsGazeAware && (gazeOnRoad || Mathf.Abs(transform.position.x) < 1f && Mathf.Abs(turnInput) < 0.7f);
            // if (gazeAwareAndOnRoad) {
            //     Debug.Log("Gaze on road");
            //     if (Mathf.Abs(transform.position.x) < 0.1f) newVelocity.x = Mathf.MoveTowards(Rigidbody.velocity.x, 0f, Time.fixedDeltaTime * m_FinalStats.Steer);
            //     else if (transform.position.x < 0f && newVelocity.x < 0f)
            //         newVelocity.x = Mathf.MoveTowards(Rigidbody.velocity.x, -Rigidbody.velocity.x, Time.fixedDeltaTime * m_FinalStats.Steer);
            //     else if (transform.position.x > 0f && newVelocity.x > 0f)
            //         newVelocity.x = Mathf.MoveTowards(Rigidbody.velocity.x, -Rigidbody.velocity.x, Time.fixedDeltaTime * m_FinalStats.Steer);
            // }
            ...
            if (GroundPercent > 0.0f)
            {
                ...
                // if (gazeAwareAndOnRoad && Mathf.Abs(angularVel.y) < 5f) {
                //     angularVel.y = 0f;
                //     Vector3 currentRotation = transform.rotation.eulerAngles;
                //     transform.rotation = Quaternion.Euler(currentRotation.x, Mathf.MoveTowardsAngle(currentRotation.y, 0f, Time.fixedDeltaTime * angularVelocitySmoothSpeed), currentRotation.z);
                // } else {
