...
public class lb_Bird : MonoBehaviour {
	...
	public float flightSpeed = 70.0f;
	...
	void SetCollideWithObjects(bool collide) {
		collideWithObjects = collide;
	}
	...
	void OnEnable () {
		...
		transform.localScale = initialScale * birdScale;
	...
	IEnumerator FlyToTarget(Transform target){
		followingTarget = target.position;
		if(Random.value < .5){
			if (flyAway1 != null) GetComponent<AudioSource>().PlayOneShot (flyAway1,.1f);
			else if (flyAway2 != null) GetComponent<AudioSource>().PlayOneShot (flyAway2,.1f);
		}else{
			if (flyAway2 != null) GetComponent<AudioSource>().PlayOneShot (flyAway2,.1f);
			else if (flyAway1 != null) GetComponent<AudioSource>().PlayOneShot (flyAway1,.1f);
		}
		...
		Vector3 awayForce = transform.forward * (50.0f / 70.0f) * flightSpeed *birdScale;
		if (transform.position.y < 5f) awayForce += transform.up * 100.0f*birdScale;
		rigidbody.AddForce(awayForce);
		float t = 0.0f;
		while (t<0.3f){
			if(!paused){
				t+= Time.deltaTime;
		...
		Vector3 vectorDirectionToTarget = (target.position-transform.position).normalized;
		...
		if(vectorDirectionToTarget.y>.5f){
			...
			while(vectorDirectionToTarget.y>.5f){
				...
				rigidbody.AddForce(transform.forward * flightSpeed*birdScale * Time.deltaTime);

				//Debug.DrawRay (transform.position,transform.forward,Color.green);

				vectorDirectionToTarget = (target.position-transform.position).normalized;//reset the variable to reflect the actual target and not the temptarget
		...
		while(transform.rotation != finalRotation){
			if(!paused){
				distanceToTarget = Vector3.Distance (transform.position,target.position);
				vectorDirectionToTarget = (target.position-transform.position).normalized;
		...
		float flyingForce = flightSpeed*birdScale;
		while(true){
			if(!paused){
				//do a raycast to see if the bird is going to hit the ground
				if (Physics.Raycast(transform.position,-Vector3.up,0.4f*birdScale)){
					if (rigidbody.velocity.y < 0f) rigidbody.velocity = new Vector3(rigidbody.velocity.x, 0.0f,rigidbody.velocity.z);
					rigidbody.AddForce(0.1f * flightSpeed * birdScale * Time.deltaTime * Vector3.up);
				}
				if (Physics.Raycast(transform.position,Vector3.up,out hit,0.15f*birdScale) && rigidbody.velocity.y > 0){
					//if the bird is going to collide with something overhead zero out vertical velocity
					if(!hit.collider.isTrigger){
				...
				if(distanceToTarget <= 1.5f*birdScale){
					...
					if(distanceToTarget < 0.5f*birdScale){
						...
						rigidbody.drag = 2.0f;
						flyingForce = flightSpeed*birdScale;
					}
		...
		// Collider[] hitColliders = Physics.OverlapSphere(target.position,0.05f*birdScale);
		...
		// 		transform.position = Vector3.SmoothDamp(transform.position,target.position,ref vel,0.5f);
		// 		t += Time.deltaTime;
		// 		distanceToTarget = Vector3.Distance (transform.position,target.position);
		// 		if(t>2.0f){
	...
	void Flee(){
		if(!dead){
			...
			// farAwayTarget += new Vector3(Random.Range (-100,100)*birdScale,10*birdScale,Random.Range (-100,100)*birdScale);
	...
	public void KillBird(){
		if(!dead){
			...
			birdCollider.size = new Vector3(0.1f,0.01f,0.1f)*birdScale;
			GetComponent<Rigidbody>().isKinematic = false;
